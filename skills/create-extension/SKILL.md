...
| 4 | **Contract** | Add matching `bytes32` constants and send functions to `contract/InstructionSender.sol` |
...
│  DataFixed parsing   JSON-decode the envelope       │
│  State locking       serialize handler calls        │
│  ActionResult        assemble response              │
│  Scripts             build, deploy, register        │
│  Tooling             contract deployment, registry  │
└─────────────────────────────────────────────────────┘
```
...
- The framework JSON-decodes the `DataFixed` envelope (OPType, OPCommand, OriginalMessage). The inner `OriginalMessage` is ABI-encoded bytes from the Solidity contract's `_message` parameter — the handler decides how to decode it.
- JSON for communication between TEE node and TEE extension (the HTTP wire format).
- Binary fields in JSON are hex strings (0x prefix optional).
- Encoding strings (e.g. OPTypes/OPCommands) as 32-byte hex fields: UTF-8 encode the string, right-pad with zero bytes to 32 bytes, then hex-encode.
  - Solidity: `bytes32("FOO")`
...
2. JSON-decode `Action.data.message` into `DataFixed` (see [references/json-schemas/datafixed.json](references/json-schemas/datafixed.json)).
...
dataFixed = jsonDecode(action.data.message)
...
| `msg` | bytes | `DataFixed.OriginalMessage` — the raw `_message` bytes passed to the Solidity contract's send function. ABI-encoded on-chain; the handler must ABI-decode it. |

**Return values:**

| Name | Type | Description |
|------|------|-------------|
| `data` | bytes | Raw bytes placed into `ActionResult.Data`. Encoding is the handler's business. |
...
- `POST /decrypt` — decrypt with the TEE node's private key
- `GET /key-info/{walletID}/{keyID}` — check key existence
- `POST /result` — post async ActionResult back to the node

## Dockerfile

The Dockerfile must run both TEE node and TEE extension. Required env vars:
...
The TEE node source: `git@gitlab.com:flarenetwork/tee/tee-node.git` (or a local directory — ask the user).
...
function send(bytes calldata _message) external payable {
    ...
    teeExtensionRegistry.sendInstructions{value: msg.value}(
        teeIds, OP_TYPE, OP_COMMAND, _message, cosigners, 0
    );
}
```
...
- **Request type** — the payload your handler receives in `msg` (after ABI-decoding). Define one per operation.
...
handleMyOp(state, msg) -> (data, status, err):
    // 1. ABI-decode msg into your request type
    request = abiDecode(msg)
    ...
    return (jsonEncode(result), 1, null)
```
...
function sendMyOp(bytes calldata _message) external payable {
    _sendInstruction(OP_TYPE_MY_OP, OP_COMMAND_MY_CMD, _message);
}
```
